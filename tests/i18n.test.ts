...
describe('createI18n', () => {
  ...
  it('should isolate locale ownership between service instances', async () => {
    ...
    const [english, arabic, isolated] = await Promise.all([
      Promise.resolve().then(() =>
        renderToStringSync(() =>
          messages.Scope({ locale: 'en', children: () => messages.text('greeting', { name: 'A' }) }),
        ),
      ),
      Promise.resolve().then(() =>
        renderToStringSync(() =>
          messages.Scope({ locale: 'ar', dir: 'rtl', children: () => messages.text('greeting', { name: 'ب' }) }),
        ),
      ),
      Promise.resolve().then(() =>
        renderToStringSync(() =>
  ...
  it('should reject unknown locale and mismatched hydration identities', () => {
    expect(() =>
      messages.Scope({ locale: 'missing' as 'en', children: 'nope' }),
    ).toThrow('Unknown i18n locale');
